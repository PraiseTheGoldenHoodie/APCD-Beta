...
/*                ArrayList newRules = new ArrayList();
            ...
            //Read and clear out.txt
            try
            {
                string outDir = comDirectory + "\\IO\\out.txt";
                System.IO.File.ReadAllLines(outDir);
                //System.IO.File.Delete(outDir);
                System.IO.File.Create(outDir);
            }
            catch (Exception outReadExcept)
            {
                Debug.WriteLine(outReadExcept+"\nFailed reading out.txt!");
            }
            updateGridView();
